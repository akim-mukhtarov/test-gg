...
def query_handler(call):
    print(call.data)
    #new_keyboard = keyboard.main
    #new_answer = answers.greetings
    if call.data == 'getPresentsList':
        '''
        new_keyboard = keyboard.endpoint
        new_answer = answers.presents_list
        '''
        scene.keyboard.update(call.data)
        scene.text.update(call.data)

    if call.data == 'getNewPresent':
        '''
        new_keyboard = keyboard.endpoint
        new_answer = answers.present_success
        '''
        scene.keyboard.update(call.data)
        scene.text.update(call.data)

    if call.data == 'presentsAll':
        '''
        new_keyboard = keyboard.endpoint
        new_answer = answers.presents_all
        '''
        scene.keyboard.update(call.data)
        scene.text.update(call.data)
